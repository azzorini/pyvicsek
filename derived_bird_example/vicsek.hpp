...
public:
	...
	Bird(double x_ = 0, double y_ = 0, double ang = 0);
	...
	void set_angle(double angle) {
		...
		while (theta > std::numbers::pi) theta -= 2*std::numbers::pi;
		while (theta < -std::numbers::pi) theta += 2*std::numbers::pi;
	}
	void set_angle_new(double angle) {
		theta_new = angle;

		while (theta_new > std::numbers::pi) theta_new -= 2*std::numbers::pi;
		while (theta_new < -std::numbers::pi) theta_new += 2*std::numbers::pi;
	}
	...
	double distance_sq(const std::shared_ptr<Bird>&) const;
...
public:
	// Default constructor
	VicsekSimulation(unsigned BirdNumber, double velocity = .03, double ang_noise = 2., double length = 7., int seed = 0);
	VicsekSimulation(const std::list< std::shared_ptr<Bird> >& birds, double velocity = .03, double ang_noise = 2., double length = 7., int seed = 0);
	...
	void update(unsigned N_steps = 1);
